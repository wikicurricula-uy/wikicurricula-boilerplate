...
process it, and save the results to a file named "results.txt".
"""
...
# Open and read the JSON configuration file and
# access the configuration data as a dictionary
...
if len(sys.argv) > 1:
    ...
    # Default to "es" if no argument is provided
...
def main():
    if WIKIPEDIA_LANGUAGE in wikipedia_config:
        ...
        print(f"Configuration not found for language '{WIKIPEDIA_LANGUAGE}'.")
        # Handle this case appropriately (e.g, exit the script).
    ...
    analysis(language, discussionURL, display_window_template,
             warnings_config, discussion_size, incipit_size, commons_gallery,
             commons_pages, itwikisource, coordinate, featured_template)


# Function to get average page views: returns visits since the
# beginning of time,average daily visits since the beginning of time,
...
def get_avg_pageviews(article_title, start, end, language):
    ...
    try:
        ...
        url = ("https://wikimedia.org/api/rest_v1/metrics/pageviews/"
               "per-article/" + language + ".wikipedia/all-access/user/" +
               article_title + "/daily/" + start + "/" + end)
...
# This function returns visits since the beginning of time,
# average daily visits since the beginning of time,
# average daily visits in the specified year
def visit(article, language):
    ...
    # calculate result1, total pageviews since the beginning of time,
    # and result2, average pageviews since de beginning of time
    ...
    try:
        ...
        url = ("https://wikimedia.org/api/rest_v1/metrics/pageviews/"
               "per-article/" + language + ".wikipedia/all-access/user/" +
               article + "/daily/" + start_all_time + "/" + end_current_year)
        html = urlopen(url).read()
        ecc = 0  # to change
        if ecc == 0:
            html = str(html)
    ...
    result3 = get_avg_pageviews(article, start_prev_year,
                                end_prev_year, language)

    # calculate result4, average pageviews from current year
    result4 = get_avg_pageviews(article, start_current_year,
                                end_current_year, language)
...
# This function counts the number of times the substring "</ref>"
# appears within the input string "text" and converts the count
# (which is an integer) into a string
def note(text):
...
# this function counts the occurrences of specific image file extensions
# within a given string and returns that count as a string.
def images(text):
    t = text.lower()
    img = str(t.count('.jpg') + t.count('.svg') + t.count('.jpeg') +
              t.count('.png') + t.count('.tiff') + t.count(
        '.gif') + t.count('.tif') + t.count('.xcf'))
    return img


'''This function relies on external web scraping
it reads the HTML content of the page and converts it's content to a string
and then extracts a specific portion of the HTML using string manipulation.
Specifically, it looks for the substring "created_at" and extracts the
following 10 characters, which should represent the creation date of the
Wikipedia article.'''
...
def first_edit(article, language):
    try:
        ...
        url = ("https://xtools.wmflabs.org/api/page/articleinfo/" +
               language + ".wikipedia.org/" + article.replace(" ", "_"))
        html = urlopen(url).read()
...
# This function analyzes and count specific types of
# tags within a given text.
# The results are returned as strings, making them
...
def warnings(t):
    ...
    if WIKIPEDIA_LANGUAGE in language_template_config:
        ...
        print(
            f"Configuration not found for language '{WIKIPEDIA_LANGUAGE}'.")
        # Handle this case appropriately (e.g, exit the script).

    tmp_to_check = sum(t.count(template) for template in
                       language_config.get("to_check"))
    tmp_synoptic = sum(t.count(template) for template in
                       language_config.get("synoptic"))
    tmp_correct = sum(t.count(template) for template in
                      language_config.get("correct"))
    tmp_curiosity = sum(t.count(template) for template in
                        language_config.get("curiosity"))
    tmp_divide = sum(t.count(template) for template in
                     language_config.get("divide"))
    tmp_sources = sum(t.count(template) for template in
                      language_config.get("sources"))
    tmp_localism = sum(t.count(template) for template in
                       language_config.get("localism"))
    tmp_pov = sum(t.count(template) for template in
                  language_config.get("pov"))
    tmp_nn = sum(t.count(template) for template in
                 language_config.get("nn"))
    tmp_recentism = sum(t.count(template) for template in
                        language_config.get("recentism"))
    tmp_manual_style = sum(t.count(template) for template in
                           language_config.get("manual_style"))
    tmp_translation = sum(t.count(template) for template in
                          language_config.get("translation"))
    tmp_wikificare = sum(t.count(template) for template in
                         language_config.get("wikificare"))
    tmp_stub = sum(t.count(template) for template in
                   language_config.get("stub"))
    tmp_stub_section = sum(t.count(template) for template in
                           language_config.get("stub_section"))
    tmp_copy_control = sum(t.count(template) for template in
                           language_config.get("copy_control"))

    sum_of_warnings = (tmp_to_check + tmp_synoptic + tmp_correct +
                       tmp_curiosity + tmp_divide + tmp_sources +
                       tmp_localism + tmp_pov)
    sum_of_warnings += (tmp_nn + tmp_recentism + tmp_manual_style +
                        tmp_translation + tmp_wikificare + tmp_stub +
                        tmp_stub_section + tmp_copy_control)

    tmp_without_sources = sum(t.count(template) for template in
                              language_config.get("without_sources"))
...
# templates from a text until no more templates are present
...
# Incipit means the opening of a manuscript, early printed book.
# Hence, incipit == introduction
...
# This function checks to if a specific template
# (defined by the featured_template variable)
...
def analysis(language, discussionURL, display_window_template,
             warnings_config, discussion_size, incipit_size,
             commons_gallery, commons_pages, itwikisource,
             coordinate, featured_template):
    # f = open('query.csv', "r") #Adding a character encoding will be required
    # for some characters in the query.csv file to avoid
    ...
    with open('query.csv', 'r', encoding=result['encoding'],
    ...
    for article in vox:
        ...
        results = open('resultati.txt', 'a', encoding='utf-8',
                       errors="replace")  # open the file in append mode
        ...
        article = article.replace(" ",
                                  "_")
        # Wikipedia page titles are case-sensitive and spaces in page
        # titles should be replaced with underscores.
        result = ""
        wikitext = ""
        ...
        try:
            ...
            url = ("https://" + language +
                   ...
                   article2 + "&prop=wikitext&formatversion=2&format=json")
        ...
        try:
            ...
            url = ("https://" + language +
                   ...
                   article2 + "&prop=pageprops&format=json&formatversion=2")

            json_url = urlopen(url)
            data = json.loads(json_url.read())
            wikidataid = data["query"]["pages"][0][
            ...
            if incipit_size:
                result = (result +
                          calculate_introduction_length(wikitext) + "\t")
            ...
            if vdq:
                result = (result +
                          vdq(wikitext, display_window_template) + "\t")

            if featured_in:
                result = (result +
                          featured_in(wikitext, featured_template) + "\t")

            if commons_gallery:
                try:
                    result = result + wikidata["entities"][wikidataid][
            ...
            if commons_pages:
                try:
                    result = result + wikidata["entities"][wikidataid][
                       "claims"]["P935"][0]["mainsnak"]["datavalue"][
                        "value"] + "\t"
            ...
            if coordinate:
                try:
                    result = result + \
                             wikidata["entities"][wikidataid]["claims"][
                                "P625"][0]["mainsnak"]["datavalue"]["value"][
                                 "latitude"] + "\t"
                    result = result + \
                        wikidata["entities"][wikidataid]["claims"]["P625"][
