...
# Open and read the JSON configuration file and access
...
def main():
    ...
    analysis(language, discussionURL, display_window_template,
             warnings_config, discussion_size,
             incipit_size, commons_gallery, commons_pages,
             itwikisource, coordinate, featured_template)


# Function to get average page views: returns visits
# since the beginning of time, average dayly visits since the
# begininning of time, average daily visits in the specified year
...
# average dayly visits since the begininning of time,
...
def visit(article, language):
    ...
    try:
        ...
        url ="https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/"
        +language+".wikipedia/all-access/user/"+ARTICLE+"/daily/"
        +START_ALL_TIME +"/" + END_CURRENT_YEAR
        html = urlopen(url).read()
        ecc = 0 # to change
        if ecc == 0:
            html = str(html)
...
# appears within the input string "text" and converts
# the count (which is an integer) into a string
def note(text):
...
def images(text):
    ...
    img = str(t.count('.jpg')+t.count('.svg')+t.count('.jpeg')
              +t.count('.png')+t.count('.tiff')+t.count('.gif')
...
Specifically, it looks for the substring "created_at" and extracts the following
10 characters, which should represent the creation date of the Wikipedia article.'''
...
# This function analyzes and count specific types of tags
# within a given text. The results are returned as strings,
# making them suitable for further processing.
...
def warnings(t):
    ...
    sum_of_warnings = tmp_to_check + tmp_synoptic + tmp_correct + tmp_curiosity
    + tmp_divide + tmp_sources + tmp_localism + tmp_pov
    sum_of_warnings += tmp_nn + tmp_recentism + tmp_manual_style + tmp_translation
    + tmp_wikificare + tmp_stub + tmp_stub_section + tmp_copy_control


    tmp_without_sources = sum(t.count(template)
                              for template in language_config.get("without_sources"))
...
# This function checks to if a specific template (defined by the
# featured_template variable) is present in the provided text.
...
def analysis(language, discussionURL, display_window_template,
   ...
   # f = open('query.csv', "r") #Adding a character encoding will
   # be required for some characters in the query.csv file
   # to avoid getting a UnicodeDecodeError.
...
# Open the file with the detected encoding
    ...
    for article in vox:
        ...
        results = open('resultati.txt', 'a', encoding='utf-8', errors="replace")
        ...
        article = article[:-1]
        article = article.replace(" ", "_")
        # Wikipedia page titles are case-sensitive and spaces
        # in page titles should be replaced with underscores.
        result = ""
        wikitext = ""
    ...
    try:
        ...
        url = "https://" + language + ".wikipedia.org/w/api.php?action=parse&page="
    ...
    try:
        ...
        url = "https://" + language + ".wikipedia.org/w/api.php?action=query&titles="
        ...
        try:
            url = "https://" + language + ".wikipedia.org/w/api.php?action=parse&page="
            + discussionURL + article2 + "&prop=wikitext&formatversion=2&format=json"
            json_url = urlopen(url)
        ...
        if first_edit:
            result += first_edit(article2, language) + "\t"

        if dimension:
            result += dimension(wikitext) + "\t"

        if images:
            result += images(wikitext) + "\t"

        if note:
            result += note(wikitext) + "\t"
        ...
        if discussion_size:
            result += dimension(wikitext_discussion) + "\t"

        if incipit_size:
            result += calculate_introduction_length(wikitext) + "\t"
        ...
        if vdq:
            result += vdq(wikitext, display_window_template) + "\t"


        if featured_in:
            result += featured_in(wikitext, featured_template) + "\t"



        if commons_gallery:
            try:
                result += wikidata["entities"][wikidataid]["claims"]["P373"]
        ...
        if commons_pages:
            try:
                result = result + wikidata["entities"][wikidataid]["claims"]
                ["P935"][0]["mainsnak"]["datavalue"]["value"] + "\t"
        ...
        if itwikisource:
            try:
                result = result + wikidata["entities"][wikidataid]["sitelinks"]
                ["itwikisource"]["title"] + "\t"
            except:
        ...
        if coordinate:
            try:
                result = result + wikidata["entities"][wikidataid]["claims"]
                ["P625"][0]["mainsnak"]["datavalue"]["value"]["latitude"] + "\t"
                result = result + wikidata["entities"][wikidataid]["claims"]
                ["P625"][0]["mainsnak"]["datavalue"]["value"]["longitude"] + "\t"
