...
# Function to get average page views: returns visits since the beginning of time,
# average dayly visits since the begininning of time, average daily visits in the specified year
...
# This function counts the number of times the substring "</ref>" appears
# within the input string "text" and converts the count (which is an integer) into a string
...
def warnings(t):
    ...
    tmp_to_check = sum(
        t.count(template) for template in language_config.get("to_check")
    )
    tmp_synoptic = sum(
        t.count(template) for template in language_config.get("synoptic")
    )
    ...
    tmp_curiosity = sum(
        t.count(template) for template in language_config.get("curiosity")
    )
    ...
    tmp_localism = sum(
        t.count(template) for template in language_config.get("localism")
    )
    ...
    tmp_recentism = sum(
        t.count(template) for template in language_config.get("recentism")
    )
    tmp_manual_style = sum(
        t.count(template) for template in language_config.get("manual_style")
    )
    tmp_translation = sum(
        t.count(template) for template in language_config.get("translation")
    )
    tmp_wikificare = sum(
        t.count(template) for template in language_config.get("wikificare")
    )
    tmp_stub = sum(t.count(template) for template in language_config.get("stub"))
    tmp_stub_section = sum(
        t.count(template) for template in language_config.get("stub_section")
    )
    tmp_copy_control = sum(
        t.count(template) for template in language_config.get("copy_control")
    )
    ...
    tmp_without_sources = sum(
        t.count(template) for template in language_config.get("without_sources")
    )
    tmp_to_clarify = sum(
        t.count(template) for template in language_config.get("clarify")
    )
...
def analysis(
    ...
    f = open(file_to_be_analysed, "r", encoding=utf_required)
    ...
    for article in articles:
        ...
        article = article[:-1]
        # Wikipedia page titles are case-sensitive and spaces in page titles should be replaced with underscores.
        article = article.replace(" ", "_")
        result = ""
        wikitext = ""
        ...
        try:
            ...
            if common_gallery:
                try:
                    result = (
                        result
                        + wikidata["entities"][wikidataid]["claims"]["P373"][0][
                            "mainsnak"
                        ]["datavalue"]["value"]
                        + "\t"
                    )
            ...
            if common_pages:
                try:
                    result = (
                        result
                        + wikidata["entities"][wikidataid]["claims"]["P935"][0][
                            "mainsnak"
                        ]["datavalue"]["value"]
                        + "\t"
                    )
            ...
            if itwikisource:
                try:
                    result = (
                        result
                        + wikidata["entities"][wikidataid]["sitelinks"]["itwikisource"][
                            "title"
                        ]
                        + "\t"
                    )
            ...
            if coordinate:
                try:
                    result = (
                        result
                        + wikidata["entities"][wikidataid]["claims"]["P625"][0][
                            "mainsnak"
