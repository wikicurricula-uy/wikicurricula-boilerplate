...
function dv1(year, the_subject, sort) {
  ...
  function loaded(data) {
    ...
    let grid = svg
      ...
      .call(
        make_y_gridlines().tickSize(-width - margin.left - margin.right - 60)
      );
    ...
    function handleMouseOver() {
      ...
      d3.selectAll(".variation").select(".line_prev").attr("opacity", 0.2);

      d3.selectAll(".variation").select(".circle_prev").attr("opacity", 0);

      // highlight
      d3.select(this).attr("opacity", 1);

      d3.select(this.previousSibling).select(".circle_prev").attr("opacity", 1);

      d3.select(this.previousSibling).select(".line_prev").attr("opacity", 1);
    }
